...
class CircularProgressBar: UIView {
    ...
    public func setProgress(to progressConstant: Double, withAnimation: Bool) {
        let timeDuration = Int(round(progressConstant))
        var progress: Double {
            get {
        ...
        if withAnimation {
            ...
            animation.duration = CFTimeInterval(timeDuration)
            foregroundLayer.add(animation, forKey: "foregroundAnimation")
            
        }
        
//        var currentTime:Double = 0
//            let timer = Timer.scheduledTimer(withTimeInterval: 50, repeats: true) { (timer) in
//                if currentTime >= 2{
//                    timer.invalidate()
//                } else {
//                    currentTime += 0.05
...
//         timer.fire()
    ...
    private func drawForegroundLayer(){
        ...
        foregroundLayer.strokeColor = AppColors.darkBlue.cgColor
    ...
    private func setForegroundLayerColorForSafePercent(){
        if Int(label.text!)! >= self.safePercent {
            self.foregroundLayer.strokeColor = AppColors.darkBlue.cgColor
        } else {
            self.foregroundLayer.strokeColor = AppColors.darkBlue.cgColor
        }
    }
